...
class PolyakStepSizeRule:
    ...
    def __init__(self, sense: OptimizationSense, target_tracker: Callable[[float], float], size_coefficient: float = 1):
    ...
    def __call__(
        self, current_value: float, multipliers: np.ndarray, direction: np.ndarray, max_size_coefficient=2.0
    ) -> float:
        target_value = self.target_tracker(current_value)
        actual_size_coef = min(max_size_coefficient, self.size_coefficient)
        return (
            actual_size_coef * self.sense * (target_value - current_value) / max(1e-3, np.linalg.norm(direction) ** 2)
        )
